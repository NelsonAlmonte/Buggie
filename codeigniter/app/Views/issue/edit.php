  ...
  <div class="card mt-4">
    <div class="card-body bg-complementary p-4">
      ...
      <form class="row gx-5" action="<?=site_url('issue/'. $project['slug'] . '/update/' . $issue['id'])?>"
        ...
        <div class="col-6 mb-4">
          <div x-data="searchSelect" x-init='query = <?=json_encode($issue['assignee_name'])?>' class="form-floating auto-complete">
          ...
          <input type="hidden" id="assignee" name="assignee" x-data='{ itemId: <?=json_encode($issue['assignee'])?> }'
            @get-item.window="itemId = $event.detail.item.id" x-model="itemId">
        </div>
        <div class="col-12 my-4">
